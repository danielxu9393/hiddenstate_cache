...
def flash_attn_fn(
    ...
    if kv_n_heads == 1:
        ...
        key_unpad = repeat_kv_for_gqa(
            key_unpad.view(batch_size, seqlen, kv_n_heads, -1), n_heads // kv_n_heads
        ).view(batch_size * seqlen, n_heads, -1)
        value_unpad = repeat_kv_for_gqa(
            value_unpad.view(batch_size, seqlen, kv_n_heads, -1), n_heads // kv_n_heads
        ).view(batch_size * seqlen, n_heads, -1)
    ...
    def forward(
        ...
        if hidden_state_cache is not None:
            if layer_idx is None:
                raise ValueError(
                    "layer_idx must be specified if hidden_state_cache is not None."
                )
            get_layer_time = time.time()
            past_encoded_hidden_states = hidden_state_cache.get_layer(layer_idx)
            # print(f"layer {layer_idx} get_layer time: ", time.time() - get_layer_time)

            # ignoring the clipping because I think it is not used by default?
            # Need to verify this!
            compute_qkv_time = time.time()
            (query, key, value) = hidden_state_cache.low_rank_qkv.compute_qkv(
                layer_idx, past_encoded_hidden_states, normed_query_hidden_states=x
            )
            # print(
            #     f"layer {layer_idx} compute_qkv time: ", time.time() - compute_qkv_time
            # )
        ...
        # print(f"layer {layer_idx} qkv time: ", time.time() - qkv_time)

        # if past_hidden_state is not None:
        #     past_qkv = self.Wqkv(past_hidden_state)  ### recompute past_key_value
        #     if self.clip_qkv:
        #         past_qkv = past_qkv.clamp(min=-self.clip_qkv, max=self.clip_qkv)
        #     (past_query, past_key, past_value) = past_qkv.split(
        #         [
        #             self.d_model,
        #             self.kv_n_heads * self.head_dim,
        #             self.kv_n_heads * self.head_dim,
        ...
        #         dim=2,
        #     )
        #     past_key = rearrange(past_key, "b s (h d) -> b h d s", h=self.kv_n_heads)
        #     past_value = rearrange(
        #         past_value, "b s (h d) -> b h s d", h=self.kv_n_heads
        #     )

        #     if past_key_value is not None and len(past_key_value) >= 2:
        #         print("old shape: ", past_key_value[0].shape, past_key_value[1].shape)
        #         print("new shape: ", past_key.shape, past_value.shape)
        #         # print(
        #         #     "difference: ",
        #         #     (past_key_value[0] - past_key, past_key_value[1] - past_value),
        #         # )
        #         difference_key = past_key_value[0] - past_key
        #         print(
        #             "num different key: ",
        #             (torch.abs(difference_key) > 0.001).sum().item(),
        #         )
        #         print("max different key: ", torch.abs(difference_key).max())
        #         difference_value = past_key_value[1] - past_value
        #         print("num different value: ", (difference_value > 0.001).sum().item())
        #         print("max different value: ", torch.abs(difference_value).max())

        #     past_key_value = (past_key, past_value)  ### override past_key_value lol
        ...
        ### Good thing qk_ln is default false??
        attn_kv_n_heads = self.kv_n_heads if hidden_state_cache is None else 1
        if hidden_state_cache is not None:
            attn_kv_n_heads = 1
            attn_fn = scaled_multihead_dot_product_attention
        else:
            attn_kv_n_heads = self.kv_n_heads
            attn_fn = self.attn_fn
        # (context, attn_weights, past_key_value) = self.attn_fn(
        timer = time.time()
        (context, attn_weights, past_key_value) = attn_fn(
            query,
            key,
            value,
            self.n_heads,
        ...
        # print(f"layer {layer_idx} attn time: ", time.time() - timer)
        # need to verify if attn_fn is ok with keys and values being same over h dimension without having to explicitly repeat
        if hidden_state_cache is not None:
            output = hidden_state_cache.low_rank_qkv.compute_output(layer_idx, context)
        else:
